...
def main():
    ...
    _w = tf.Variable(tf.transpose(codebook))
    _ema_cluster_size = tf.Variable(init(shape=(num_codes)), dtype=tf.float32, trainable=False) # _ema_cluster_size
    _ema_w = tf.Variable(_w.read_value(), dtype=tf.float32, trainable=False) # _ema_w
